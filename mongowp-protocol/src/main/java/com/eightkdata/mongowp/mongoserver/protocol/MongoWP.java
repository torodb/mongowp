...
public class MongoWP {
    ...
    public enum ErrorCode {
         ...
         */
        public static ErrorCode fromErrorCode(int id) throws IllegalArgumentException{
            for (ErrorCode error : MongoWP.ErrorCode.values()) {
                if (error.getErrorCode() == id) {
                    return error;
                }
            }
            throw new IllegalArgumentException("There is no error with the error code '" + id + "'");
        }

        public String getErrorMessage() {
            return ResourceBundle
                    .getBundle("com/eightkdata/mongowp/mongoserver/protocol/Bundle")
                    .getString(errorMessageToken);
        }
    }
}
