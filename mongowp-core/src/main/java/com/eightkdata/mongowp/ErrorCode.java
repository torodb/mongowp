...
public enum ErrorCode {
  ...
  NEW_REPLICA_SET_CONFIGURATION_INCOMPATIBLE(103,
      "error.code.new.replica.set.configuration.incompatible"),
  ...
  BACKGROUND_OPERATION_IN_PROGRESS_FOR_DATABASE(12586,
      "error.code.background.operation.in.progress.for.database"),
  BACKGROUND_OPERATION_IN_PROGRESS_FOR_NAMESPACE(12587,
      "error.code.background.operation.in.progress.for.namespace"),
   ...
   */
  public static ErrorCode fromErrorCode(int code) throws IllegalArgumentException {
    return fromErrorCodeOpt(code).orElseThrow(
        () -> new IllegalArgumentException("There is no error with the error code '" + code + "'"));
  }

  public static Optional<ErrorCode> fromErrorCodeOpt(int code) {
    return Stream.of(values()).filter(ec -> ec.getErrorCode() == code).findAny();
  }
